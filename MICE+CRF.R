...
mse=foreach(n_i=c(0.1,0.2,0.3,0.4))%:% #inicializamos con el porcentajo de datos faltantes
  foreach(r=c(1:1),.packages=c("mice","party","tidyverse"))%dopar%{
   ...
   crf=imp_entre %>% lapply(function(x){cforest(formula=yi~x1+x2+x3+x4+x5+x6+x7+x8+x9+x10,data = x,controls = cforest_unbiased(ntree = 500, mtry = min(5,ncol(datos)-1), maxsurrogate = min(3, ncol(datos)-1)))}) #Aplicando rf
...
saveWorkbook(wb, file = "CRF-MICE.xlsx", overwrite = TRUE)
